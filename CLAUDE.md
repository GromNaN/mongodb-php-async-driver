...
- **`WriteResult` count fields for unacknowledged writes**: the CRUD spec says fields are "undefined" for unacknowledged writes but doesn't mandate null vs 0. The actual behaviour is a libmongoc detail replicated in `OperationExecutor::executeBulkWrite()`:
  - **Explicit `WriteConcern(0)` passed in options** → libmongoc receives `w=0` on the bulk operation → `mongoc_write_concern_is_acknowledged(w=0) = false` → count fields are **omitted** from the reply BSON → `nInserted` etc. show as **`NULL`** in `var_dump(WriteResult)`.
  - **`w=0` inherited from the URI / Manager default** (no `writeConcern` key in options) → libmongoc's bulk operation keeps a `NULL` write concern → `mongoc_write_concern_is_acknowledged(NULL) = true` → count fields are **included** with value 0 → `nInserted` etc. show as **`int(0)`**.
  - In both cases `isAcknowledged()` returns `false`. The distinction is tracked via the `$writeConcernExplicit` boolean parameter threaded from `Manager::executeBulkWrite()` down to `OperationExecutor::executeBulkWrite()`.
